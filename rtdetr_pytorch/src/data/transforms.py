...
# ==========================================
# ★ 导师修复：兼容不同版本 torchvision 的 ToImageTensor
# ==========================================
try:
    # 尝试加载新版 API (torchvision >= 0.17)
    ToImageTensor = register(T.ToImage)
except AttributeError:
    # 如果失败，回退到旧版 API (torchvision < 0.17)
    ToImageTensor = register(T.ToImageTensor)
# ==========================================

# ==========================================
# ★ 导师修复：多版本 torchvision API 动态映射
# ==========================================
try:
    ConvertDtype = register(T.ConvertDtype)
except AttributeError:
    ConvertDtype = register(T.ToDtype)  # 适配 torchvision >= 0.17

try:
    SanitizeBoundingBox = register(T.SanitizeBoundingBox)
except AttributeError:
    SanitizeBoundingBox = register(T.SanitizeBoundingBoxes) # 适配 torchvision >= 0.17
